...
export type CompletedEventsQuery = (
  ...
  & { Events: Array<(
    { __typename?: 'Events' }
    & Pick<Events, 'id' | 'title' | 'image' | 'start_time' | 'description' | 'video_url'>
    & { User: (
      { __typename?: 'Users' }
...
export type HitsDetailFragment = (
  { __typename?: 'Hits' }
  & Pick<Hits, 'id' | 'image_front' | 'image_back' | 'player' | 'card_number' | 'parallel' | 'insert' | 'rookie_card' | 'memorabilia' | 'autograph' | 'numbered' | 'user_id'>
  & { User: (
    { __typename?: 'Users' }
    & Pick<Users, 'username'>
  ), Product: (
...
export const CompletedEventsDocument = gql`
    query CompletedEvents($breakerId: String_comparison_exp, $startDate: timestamptz_comparison_exp, $endDate: [Events_bool_exp!], $userId: BreakProductItems_bool_exp, $limit: Int) {
  Events(
    limit: $limit
    order_by: {start_time: desc}
    where: {status: {_eq: COMPLETED}, archived: {_eq: false}, user_id: $breakerId, start_time: $startDate, _and: $endDate, Breaks: {BreakProductItems: $userId}}
  ) {
    id
    title
    image
...
export const HitsScreenDocument = gql`
  ...
  Hits(
    where: {user_id: $userHitsFilter, archived: {_eq: false}, published: {_eq: true}, _or: [{player: {_ilike: $searchInput}}, {memorabilia: {_ilike: $searchInput}}, {parallel: {_ilike: $searchInput}}, {insert: {_ilike: $searchInput}}, {Product: {_or: [{year: {_ilike: $searchInput}}, {category: {_ilike: $searchInput}}, {manufacturer: {_ilike: $searchInput}}, {brand: {_ilike: $searchInput}}, {series: {_ilike: $searchInput}}]}}]}
    order_by: [{created_at: desc}, {id: desc}]
    offset: $offset
    limit: $limit
  ) {
